			...
			<div class=" ">
				<div class="row">
					<div id="" class="col-md-7">
						...
						<map name="map" class="seatmap">
                            ...
                            @foreach($coords as $key => $ticket)
                                <area 
                                    data-seat="{{ in_array($key,$zonas_disponibles) ? $key : 'sold' }}"  
                                    class="selected_zone" 
						...
						<div class="seat-label">
							<ul>
								<li><img src="{{ asset('assets/images/select_seat/available.png') }}" alt="available" > Available</li>
								<li><img src="{{ asset('assets/images/select_seat/sold.png') }}" alt="sold" > Sold Out</li>
								<li><img src="{{ asset('assets/images/select_seat/selected.png') }}" alt="selected" > Selected</li>
							</ul>
						</div> 
					</div>
                    ...
                    @foreach($tickets->where('is_hidden', false) as $ticket)
                                <tr style="cursor: pointer" data-seat="{{$ticket->seat_zone}}" class="selected_zone" data-toggle="modal" data-target="#myModal">
                                    <td class="">
                                    ...
                                    <td class=""><span class="ver_asientos_marcados_seccion_comprar_zona_{{ $ticket->seat_zone }}">0</span></td>
                                    <td class="text-right">
                                        <span
                                            ...
                                            <table class="table seat-row">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        {{-- <td class="text-center">{{$ticket->max_per_person}} / {{$ticket->quantity_available}}</td> --}}
                                                        <td class="text-center"><span class="ver_asientos_marcados_seccion_comprar_zona_{{ $ticket->seat_zone }}">0</span></td>
                                                    </tr>
                                                </tbody>
                                            </table>	 
                                        </div>
                                            ...
                                            <div class="row m-5">
                                                ...
                                                @else
                                                    <div class="col-md-12" style="padding: 50px" >
                                                        ...
                                                        <select name="ticket_{{$ticket->id}}" data-id="{{$ticket->id}}" data-seatzone="{{ $ticket->seat_zone }}" class="form-control select-nro-seat" style="text-align: center">
                                                            @if ($tickets->count() > 1)
                                                ...
                                                @foreach ($ticket_seats as $key_file => $seats)
                                                    ...
                                                    <div class="col-md-12">
                                                        ...
                                                        <li class="seat seatSelect {{ $seat->is_available == 3 ? "btns-danger": "btns-info" }}" style="width: 25px; padding-left: 1px;"
                                                            id="{{ $seat->id }}" 
                                                        ...
                                                        {{-- <a id="{{ $seat->id }}" data-asiento="{{ $seat->seat() }}"
                                                            ...
                                                            data-seatzone="{{ $ticket->seat_zone }}" {{ $seat->is_available == 3 ? "disabled"
                                                            : "" }} class="seat" style="line-height: 0 !important; margin: 0px 1px 0px 1px;padding:5px;" href="javascript:void(0)"
                                                            title="{{ "Fila: ".$seat->row." - Asiento: ".$seat->column }}">
                                                           
                                                            <svg style="width: 20px" class="icon-seat" viewBox="0 0 24 20"><path fill="#ffc526" class="st0" d="M22.7,0H21c-0.7,0-1.3,0.6-1.3,1.4v0.4v2.6c0-2-1.6-3.7-3.6-3.7H8c-2,0-3.6,1.7-3.6,3.7V1.8V1.4 C4.4,0.6,3.8,0,3,0H1.3C0.6,0,0,0.6,0,1.4v0.4v11.3v0.5c0,1,0.8,1.8,1.8,1.8h0.1c0.1,0,0.2,0,0.3,0h0.2l0,0H3v2.7 C3,19.1,3.8,20,4.9,20h14.3c1.1,0,1.9-0.9,1.9-1.9v-2.7h0.6l0,0h0.8c0.9-0.1,1.5-0.9,1.5-1.8v-0.5V1.8V1.4C24,0.6,23.4,0,22.7,0z"></path></svg>
